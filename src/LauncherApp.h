...
class LauncherApp {
 public:
  explicit LauncherApp(HANDLE benchmarkPresentedEvent = nullptr);
  ~LauncherApp();
 ...
 private:
  ...
  Microsoft::WRL::ComPtr<ID2D1Bitmap> LoadBitmapFromFile(const std::wstring& path);
  Microsoft::WRL::ComPtr<ID2D1Bitmap> LoadBitmapFromShellTarget(const std::wstring& target);
  void QueueShellIconLoad(const std::wstring& target);
  void HandleLoadedShellIcon(LPARAM payload);
  ...
  bool installedApplicationsLoaded_ = false;
  ...
  std::unordered_map<std::wstring, HBITMAP> shellIconSources_;
  std::unordered_set<std::wstring> pendingIconTargets_;
  std::unordered_set<std::wstring> failedIconTargets_;
  std::vector<std::thread> iconLoaderThreads_;
